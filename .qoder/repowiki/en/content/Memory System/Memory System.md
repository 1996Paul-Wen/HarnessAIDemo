...
## Introduction
This document explains the Memory System sub-component that implements a three-tiered memory architecture inspired by human cognition:
- Short-term memory for recent conversation context (bounded buffer).
- Long-term memory for persistent knowledge with TF-IDF retrieval and JSON persistence.
- Hybrid memory that combines both to build rich, relevant prompts for the agent.

The system integrates tightly with Context Management and Agent execution to influence how agents behave across turns and sessions. It provides configuration options for capacity, retention policies, and retrieval strategies, and includes practical guidance for handling overflow, accuracy, and performance at scale.

## Project Structure
The memory subsystem is organized into focused modules:
- Base abstractions and data model
- Short-term memory with bounded FIFO buffer
- Long-term memory with TF-IDF search and JSON persistence
- Hybrid memory orchestrating short-term and long-term
...
E["ContextManager<br/>build_messages()"]
F["BaseAgent<br/>run()"]
end
A --> B
A --> C
...
- LongTermMemory: Persistent store backed by JSON; TF-IDF scoring for retrieval; load/save on mutations.
- HybridMemory: Orchestrates short-term and long-term; builds combined context strings for prompts.

Key behaviors:
- Short-term memory enforces capacity via deque(maxlen=capacity), dropping oldest items when full.
- Long-term memory persists all additions and supports retrieval by query with top_k results.
...
The memory system sits between the Agent loop and the LLM prompt assembly. The Context Manager composes the final prompt by combining:
...
- Search: Implements TF-IDF scoring:
  - Tokenizes query and item contents into words.
...
- Query tokenization splits on whitespace and lowercases.
...
#### Sequence Diagram: TF-IDF Search Flow
```mermaid
flowchart TD
Start(["Search Entry"]) --> CheckEmpty{"Any items?"}
CheckEmpty --> |No| ReturnEmpty["Return []"]
CheckEmpty --> |Yes| Tokenize["Tokenize query into terms"]
...
### HybridMemory: Combining Short-Term and Long-Term
...
- Agent depends on Context Manager and memory to drive the execution loop.
...
- Long-term memory:
  - TF-IDF retrieval scales linearly with the number of stored items; consider chunking or indexing for very large stores.
...
- Retrieval accuracy in long-term:
  ...
  - Fixes: Improve query normalization, add synonyms, or augment metadata; consider embedding-based retrieval for semantic similarity.
...
## Conclusion
The Memory System provides a cognitively inspired, layered approach to maintaining conversational continuity and knowledge reuse:
- Short-term memory ensures fresh context with bounded storage.
- Long-term memory offers persistent, searchable knowledge via TF-IDF and JSON.
- Hybrid memory orchestrates both to produce high-quality prompts that guide agent behavior effectively.

By tuning capacity, retention, and retrieval parameters, and by integrating closely with Context Management and the Agent loop, teams can build robust, scalable assistants that remember what matters and respond with relevant, coherent answers.
...
- Choose a durable storage_path for long-term memory and ensure backups.
...
- Persisting and searching long-term knowledge:
  - Add user/assistant messages to long-term and retrieve top_k relevant items via TF-IDF.
  - Reference: [long_term.py:32-68](file://harness/memory/long_term.py#L32-L68)
...
- Integrating with agent loop:
  - Context Manager injects memory context and persists assistant responses.
  - Reference: [manager.py:61-108](file://harness/context/manager.py#L61-L108), [base.py:97-165](file://harness/agent/base.py#L97-L165)

[No additional sources needed since these references point directly to analyzed files]